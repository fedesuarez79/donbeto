...
while ($fila = mysql_fetch_object($result)) {
  ...
  echo "<td align='center'>Stock</td></tr>";
  ...
  while ($fila2 = mysql_fetch_object($result2)) {
	...
	$fila2->precio_venta=round($fila2->precio_venta,0);
      }else if($fila2->precio_venta>1){
	$fila2->precio_venta=round($fila2->precio_venta,1);
      }else{
	$fila2->precio_venta=round($fila2->precio_venta,4);
      }
      if(!$fila2->depende_id){
	if(!$fila2->precio_venta){
	  echo "<td align='center'>Consultar</td></tr>";
	}else{
  	  echo "<td align='center'>$".$fila2->precio_venta."</td><td align='center'>".$fila2->stock."</td></tr>";
	}
      }else{
	...
	echo "<td align='center'>$".round($fila3[1]*$fila3[2]*(1+$fila2->marcado)/$fila2->fraccion,3)."</td><td align='center'>".$fila2->stock."</td></tr>";
